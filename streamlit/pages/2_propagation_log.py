...
try:
    ...
    st.error(
        "エラー: `datagen.data_utils` モジュールが見つかりません。ディレクトリ構造を確認してください。"
    )
    st.stop()
...
if not os.path.exists(SAVE_DIR_PATH):
    ...
    st.info(
        "`1_graph_visualization.py` などのページでグラフを保存すると、自動的に作成されます。"
    )
    st.stop()
...
if active_graph:
    ...
    seed_nodes_input = st.sidebar.text_input(
        "シードノード (カンマ区切り)", value=default_seeds, key="prop_log_seed_input"
    )
    ...
    if seed_nodes_input.strip():
        try:
            potential_seeds = [
                int(s.strip()) for s in seed_nodes_input.split(",") if s.strip()
            ]
    ...
    if st.sidebar.button("伝播シミュレーション実行", disabled=not parsed_seeds):
        ...
        if st.session_state.prop_log_latest_results:
            st.session_state.prop_log_previous_results = (
                st.session_state.prop_log_latest_results.copy()
            )
...
def display_simplified_results(results, title="前回の結果"):
    ...
    with st.container(border=True):
        ...
        st.text_area(
            "Final Nodes",
            value=", ".join(map(str, sorted(list(results["final_activated"])))),
            height=100,
...
def display_full_results(results, graph, title="最新の結果"):
    ...
    with st.container(border=True):
        ...
        chosen_step = 0
        if max_step > 0:
            chosen_step = st.slider(
                "表示するステップを選択:", 0, max_step, max_step, key=f"slider_{title}"
            )
        ...
        for node in graph.nodes():
            ...
            vis_nodes.append(
                Node(id=str(node), label=str(node), color=color, size=size, shape=shape)
            )
        ...
        for u, v, d in graph.edges(data=True):
            ...
            is_used = not log_df[
                (log_df["source"] == u)
                & (log_df["target"] == v)
                & (log_df["step"] <= chosen_step)
            ].empty
            if is_used:
                color, width = "blue", 2.5
            elif u in active_at_step and v not in active_at_step:
                color = "#FFC0CB"
            vis_edges.append(
                Edge(
                    source=str(u),
...
if latest_results and previous_results:
    ...
    # シミュレーション未実行
    st.info(
        "サイドバーでシードノードを設定し、「伝播シミュレーション実行」ボタンを押してください。"
    )
